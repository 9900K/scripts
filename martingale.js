var config = {
  ...
  target: { label: "Target", type: "multiplier", value: 11 },               // target multiplier
  betMultiplier: { label: "Bet multiplier", type: "multiplier", value: 1.15 } // what to multiply the bet size by when we lose a wager
}
...
while (true) {
  ...
  if (multiplier < config.target.value) { // loss
    ...
    if (this.balance<str.balance) stop betting ();
